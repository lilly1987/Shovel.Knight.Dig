...
namespace BepInPluginSample
{
    ...
    public class Sample : BaseUnityPlugin
    {
        ...
        private static ConfigEntry<bool> noHealth;
        private static ConfigEntry<bool> goldInRun;
        private static ConfigEntry<bool> noMagicPoints;
        ...
        public void Awake()
        {
            ...
            noHealth = Config.Bind("game", "noHealth", true);
            goldInRun = Config.Bind("game", "goldInRun", true);
            noMagicPoints = Config.Bind("game", "noMagicPoints", true);
        ...
        public virtual void WindowFunction(int id)
        {
            ...
            else // 열렸을때
            {
                ...
                if (GUILayout.Button($"goldInRun {goldInRun.Value}")) { goldInRun.Value = !goldInRun.Value; }
                if (GUILayout.Button($"noHealth {noHealth.Value}")) { noHealth.Value = !noHealth.Value; }
                if (GUILayout.Button($"noMagicPoints {noMagicPoints.Value}")) { noMagicPoints.Value = !noMagicPoints.Value; }
                GUILayout.Label($"---");
                if (GUILayout.Button($"Health = 8 ; {StageController.Health}")) { StageController.Health=8; }
                if (GUILayout.Button($"goldInRun +=100000 ; {StageController.goldInRun}"))
                {
                    StageController.goldInRun += 100000;
                }
                if (GUILayout.Button($"totalGold +=100000 ; {StageController.totalGold}")) {
                    StageController.totalGold += 100000;
                    SaveManager.SetInt("total gold", StageController.totalGold);
                    SaveManager.Save();
                }
                GUILayout.Label($"---");
                GUILayout.Label($"goldInRun {StageController.goldInRun}");
                GUILayout.Label($"prevGoldInRun {StageController.prevGoldInRun}");
                GUILayout.Label($"totalGold {StageController.totalGold}");
        ...
        [HarmonyPatch(typeof(RelicCollectible), "MagicPoints", MethodType.Setter)]
        [HarmonyPrefix]
        public static void MagicPoints(ref int __0)
        {
            if (!noMagicPoints.Value)
            {
                return;
            }
            __0 =99;
        ...
        [HarmonyPatch(typeof(StageController), "Health", MethodType.Setter)]
        [HarmonyPrefix]
        public static void Health(ref int __0)
        {
            if (!noHealth.Value)
            {
                return;
        ...
        //public static void ResetGameFromStart(bool backToTitleScreen = true, bool saveDigResults = true, bool resetCutsceneStatics = true)
        [HarmonyPatch(typeof(StageController), "ResetGameFromStart",typeof(bool),typeof(bool),typeof(bool))]
        [HarmonyPrefix]
        public static void Health(bool backToTitleScreen , bool saveDigResults , bool resetCutsceneStatics )
        {
            if (!goldInRun.Value)
            {
                return;
